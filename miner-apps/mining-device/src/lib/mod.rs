...
use stratum_apps::{
    ...
    stratum_core::{
        ...
        handlers_sv2::{
            HandleCommonMessagesFromServerSync, HandleMiningMessagesFromServerSync,
            SupportedChannelTypes,
        },
        mining_sv2::*,
        noise_sv2::Initiator,
        parsers_sv2::{Mining, MiningDeviceMessages, Tlv},
    },
};
...
/// Error type for mining device handlers
#[derive(Debug)]
pub enum MiningDeviceError {
    /// Parser error
    Parser(String),
...
impl stratum_apps::stratum_core::handlers_sv2::HandlerErrorType for MiningDeviceError {
    fn parse_error(e: stratum_apps::stratum_core::parsers_sv2::ParserError) -> Self {
        MiningDeviceError::Parser(format!("{:?}", e))
    }

    fn unexpected_message(ext_type: u16, msg_type: u8) -> Self {
        MiningDeviceError::UnexpectedMessage(ext_type, msg_type)
    }
}

/// Generator of unique IDs for channels and sequence numbers.
...
impl SetupConnectionHandler {
    ...
    pub async fn setup(
        ...
        // Receive and handle response using new handler API
        let mut incoming: StdFrame = match receiver.recv().await {
            Ok(frame) => match frame {
                EitherFrame::Sv2(f) => f,
                EitherFrame::HandShake(_) => panic!("Unexpected handshake frame"),
            },
            Err(e) => panic!("Failed to receive frame: {:?}", e),
        };
        let header = incoming.get_header().unwrap();
        let payload = incoming.payload();

        // Use the new handler API
        self_
            .safe_lock(|handler| {
                handler.handle_common_message_frame_from_server(None, header, payload)
            })
            .unwrap()
            .unwrap();
    }
...
impl HandleCommonMessagesFromServerSync for SetupConnectionHandler {
    type Error = MiningDeviceError;
    ...
    fn handle_setup_connection_error(
        &mut self,
        _server_id: Option<usize>,
        _m: stratum_apps::stratum_core::common_messages_sv2::SetupConnectionError,
        _tlv_fields: Option<&[Tlv]>,
    ) -> Result<(), Self::Error> {
        error!("Setup connection error");
        Err(MiningDeviceError::Custom(
            "Setup connection error".to_string(),
        ))
    }
    ...
    fn handle_channel_endpoint_changed(
        &mut self,
        _server_id: Option<usize>,
        _m: stratum_apps::stratum_core::common_messages_sv2::ChannelEndpointChanged,
        _tlv_fields: Option<&[Tlv]>,
    ) -> Result<(), Self::Error> {
        todo!()
    }
...
impl Device {
    ...
    async fn start(
        ...
        loop {
            ...
            // Use the new handler API
            let result = self_mutex.safe_lock(|device| {
                device.handle_mining_message_frame_from_server(None, header, payload)
            });

            match result {
                Ok(Ok(())) => {
                    // Check if we should notify mining thread of new work
                    let notify_changes_to_mining_thread = self_mutex
                        .safe_lock(|s| s.notify_changes_to_mining_thread.clone())
                        .unwrap();
                    if notify_changes_to_mining_thread.should_send
                        && (message_type == MESSAGE_TYPE_NEW_MINING_JOB
                            || message_type == MESSAGE_TYPE_MINING_SET_NEW_PREV_HASH
                            || message_type == MESSAGE_TYPE_SET_TARGET)
                    {
                        notify_changes_to_mining_thread
                            ...
                            .unwrap();
                        self_mutex
                            .safe_lock(|s| s.notify_changes_to_mining_thread.should_send = false)
                            .unwrap();
                    }
                }
                Ok(Err(e)) => {
...
impl HandleMiningMessagesFromServerSync for Device {
    type Error = MiningDeviceError;
