...
use stratum_apps::{
    custom_mutex::Mutex,
    stratum_core::{codec_sv2::StandardEitherFrame, parsers_sv2::AnyMessage as JdsMessages},
};
use tokio::{select, task};
use tracing::{error, info, warn};

/// Macro for handling results with error propagation via status channel.
#[macro_export]
macro_rules! handle_result {
    ($sender:expr, $result:expr) => {
        match $result {
            Ok(val) => val,
            Err(e) => {
                let _ = $crate::status::handle_error(&$sender, e.into()).await;
                return;
            }
        }
    };
...
/// Uses StandardSv2Frame which is compatible with the Connection's EitherFrame.
pub type StdFrame = stratum_apps::stratum_core::codec_sv2::StandardSv2Frame<Message>;

/// Type alias for the either frame type (Sv2 or Handshake).
pub type EitherFrame = StandardEitherFrame<Message>;

/// Type alias for SV2 frames used in communication.
pub type Sv2FrameType = stratum_apps::utils::types::Sv2Frame;
...
impl JobDeclaratorServer {
    ...
    pub async fn start(&self) -> Result<(), JdsError> {
        ...
        let url = format!("{}:{}", config.core_rpc_url(), config.core_rpc_port());
                ...
                if let Err(err) = result {
                    match err {
                        JdsMempoolError::EmptyMempool => {
                            if last_empty_mempool_warning.elapsed().as_secs() >= 60 {
                                warn!("{:?}", err);
                                warn!("Template Provider is running, but its mempool is empty");
                                last_empty_mempool_warning = std::time::Instant::now();
                            }
                        }
                        _ => {
